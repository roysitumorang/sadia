	...
	Migrations[1748334365969169046] = func(ctx context.Context, tx pgx.Tx) (err error) {
		...
		if _, err = tx.Exec(
			...
			`CREATE TABLE sessions (
				...
				, store_id character(36) NOT NULL REFERENCES stores (id) ON UPDATE CASCADE ON DELETE CASCADE
				, date date NOT NULL
		...
		if _, err = tx.Exec(
			...
			`CREATE TABLE products (
				id character(36) NOT NULL DEFAULT uuidv7() PRIMARY KEY
				, company_id character(36) NOT NULL REFERENCES companies (id) ON UPDATE CASCADE ON DELETE CASCADE
				, category_id character(36) REFERENCES product_categories (id) ON UPDATE CASCADE ON DELETE SET NULL
				, name character varying NOT NULL
		...
		if _, err = tx.Exec(
			...
			`CREATE TABLE transaction_line_items (
				id character(36) NOT NULL DEFAULT uuidv7() PRIMARY KEY
				, transaction_id character(36) NOT NULL REFERENCES transactions (id) ON UPDATE CASCADE ON DELETE CASCADE
				, product_id character(36) NOT NULL REFERENCES products (id) ON UPDATE CASCADE ON DELETE CASCADE
